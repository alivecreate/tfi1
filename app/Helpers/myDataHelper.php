...
function getProductsFromSubCategory($id){
    ...
    if(isset($productId)){
        return Media::where('media_id', $productId->id)->orderBy('id','desc')->get();
    }else{
        return [];
    }
...
function checkIsProductAvailable($id){
  ...
  function getSubCategoryImages($id, $limit=0, $order = 'DESC'){
    ...
    if($limit == 0){
        // dd($id);
        return Media::where('media_id', $id)->orderBy('id',$order)->get();
    }
    // elseif($limit == 1){
    //     return Media::where('media_id', $id)->orderBy('id',$order)->first();
    // }
    else{
        // dd(Media::where('media_id', $id)->limit($limit)->orderBy('id',$order)->get());

        return Media::where('media_id', $id)->limit($limit)->orderBy('id',$order)->get();
    }
}
...
function getMainCategories($skip = 0, $limit = null){
    ...
    if(isset($limit)){
        ...
        return DB::table('categories')->where(['status' => 1, 'parent_id' => 0])->orderBy('item_no')
        ->limit($limit)->get();
    }else{
        return DB::table('categories')->where(['status' => 1, 'parent_id' => 0])->orderBy('item_no')
        ->get();
